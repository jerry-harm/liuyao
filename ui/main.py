    ...
    BoxLayout:
        ...
        BoxLayout:
            ...
            Button:
                ...
                size_hint: 0.1,1
                on_press: 
                    root.manager.current = 'zhouyi'
                    root.dispatch('on_zhouyi', original.gua.gua_ci(), changed.gua.gua_ci())
        BoxLayout:
            orientation: 'horizontal'
            SixGodWidget:
                id: sixgod
...
class MainScreen(Screen,EventDispatcher):
    def __init__(self, **kwargs):
        self.register_event_type('on_zhouyi')
        super(MainScreen,self).__init__(**kwargs)
    
    def on_zhouyi(self,*args):
        pass
    ...
    def format_output(self,time,empty,original:六爻,changed:六爻,sixgod:List[str],change:List[str],*args) -> str:
        ...
        for i in reversed(range(6)):
            ...
            if changed.卦象[i]:
                ...
                c_yao=YIN
            gua+=sixgod[i]+\
                o_yao+original.六亲[i]+original.甲子[i]+original.五行[i]+o_shi+\
                ('\u3000' if not change[i] else change[i])+\
                c_yao+changed.六亲[i]+changed.甲子[i]+changed.五行[i]+c_shi+"\n"
        return f"{time}\n{empty}\n"+gua
